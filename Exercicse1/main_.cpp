...
void renderScene()
{
	...
	glDrawElements(GL_TRIANGLES, faceCount * 3, GL_UNSIGNED_INT, (GLint*)faceIndex);

	glUniformMatrix4fv(UniformId, 1, GL_TRUE, M);
	glDrawElements(GL_TRIANGLES, faceCount * 3, GL_UNSIGNED_INT, (GLvoid*)0);

	//glUniformMatrix4fv(UniformId, 1, GL_TRUE, quad_normals);
	//glDrawElements(GL_TRIANGLES, faceCount * 3, GL_UNSIGNED_INT, (GLvoid*)0);
...
void display()
{
	...
	gluPerspective(45, 1.333f, 0.1f, 1000);
...
void reshape(int w, int h)
{
	...
	//glMatrixMode(GL_PROJECTION);
	//glLoadIdentity();
	//gluPerspective(45, 1.333, 0.1f, 100);
	//glMatrixMode(GL_MODELVIEW);
	//glLoadIdentity();
	//glClear(GL_COLOR_BUFFER_BIT);
